...
# Row-1 = non-SOF (A = 0); Row-2 = SOF (A = 1)
risk0 <- tp$est[1, ]            # non-SOF cumulative risk
risk1 <- tp$est[2, ]            # SOF      cumulative risk
rd    <- risk1 - risk0          # risk difference SOF − non-SOF

## 9.2  Pointwise standard errors
# tp$var is a 2×2×T array: Var/​Cov of (risk0, risk1) at each time-point
var_rd <- sapply(seq_along(times), function(i) {
  V <- tp$var[ , , i]
  V[2, 2] + V[1, 1] - 2 * V[1, 2]          # delta-method: Var(r1-r0)
})
sd_rd  <- sqrt(var_rd)
