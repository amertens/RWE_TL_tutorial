...
##----------------------------------------------------------------------------
## Data‑generating function for HCV‑AKI simulations
##   • Supports well‑specified vs misspecified PS (complexity flag)
##   • Supports proportional vs non‑proportional hazards (np_hazard flag)
##   • Supports independent vs informative admin censoring (dep_censor flag)
##   • NEW: supports informative treatment‑switch censoring driven by
##           treatment status & baseline CKD (*switch_on* flag)
##----------------------------------------------------------------------------
...
lambda_sw0      = 0.03        # baseline switch hazard (1/days)
gamma_A         = 0.60        # log‑HR for treatment on switch
gamma_ckd       = 0.40        # log‑HR per CKD on switch
## misc
censor_base     = 1/10000       # admin censoring rate (1/days)
treat_override  = c("simulate")
add_missing     = FALSE
impute          = FALSE
seed            = NULL
...
true_HR_0_365 <- true_hr_period(start = 0,
                               end   = 365,
                               np_hazard = TRUE,   # if using NP hazards
                               HR_early = 1.25,
                               HR_late  = 0.7)
